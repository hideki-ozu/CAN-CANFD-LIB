...
- **受信側の検証**:
  - SD: サービス／メソッドID、バージョン、メッセージ種別、長さ、エントリ長、オプション長、オプション参照の範囲を検査し、不正なものは破棄して数えます。元の実装は参照範囲を検査していませんでした。
...
1. **コーデック自己試験（19項目）**: 仕様表から手計算した正解バイト列と、シリアライザ出力の一致を確認します（Find、3種類のオプション付き Offer、SubscribeEventgroup、カウンタ付き Nack、通知）。INET でのデシリアライズと往復変換、未知オプションの読み飛ばし、不正入力7種の拒否も確認します。
