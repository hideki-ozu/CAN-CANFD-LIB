...
- **ACF CAN / CAN Briefのv1形式（0x01/0x02）のみ**: IEEE 1722-2025のCAN V2（0x21/0x22）、CAN XL（0x11/0x12）、LIN・FlexRay等の他ACF型は送信せず、受信時は読み飛ばします。AAF/CVF/CRF等のメディア形式、AVTP over UDP、IEEE 1722.1（AVDECC）、MAAPによるアドレス自動割当はありません（アドレスは静的設定）。
