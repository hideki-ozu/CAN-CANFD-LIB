...
- Success requires native CAN/CAN-FD tests, all four mixed network configurations the IEEE 1722 AVTP tests (tests/test_avtp.py), the SOME/IP tests (tests/test_someip.py) and the E2E/SecOC tests (tests/test_protection.py), not only a successful build. Inspect results/canfd/report.json, results/mixed/verification.json, results/avtp/report.json, results/someip/report.json and results/protection/report.json.
